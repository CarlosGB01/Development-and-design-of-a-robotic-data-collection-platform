...
void callback(const ImageConstPtr& image, const CameraInfoConstPtr& camara_info, const PointCloud2ConstPtr& point_cloud)
{
  ...
  std::strftime(output, output_size, "%Y_%m_%d_%H_%M_%S", timeinfo);
...
int main(int argc, char** argv)
{
  ...
  root_output_dir = ros::package::getPath("data_sync");

  // Create the subscribers
  message_filters::Subscriber<Image> image_sub(nh, "/pylon_camera_node/image_rect", 1);
  message_filters::Subscriber<CameraInfo> camera_info_sub(nh, "/pylon_camera_node/camera_info", 1);
  message_filters::Subscriber<PointCloud2> point_cloud_sub(nh, "/neuvition_cloud", 1);
